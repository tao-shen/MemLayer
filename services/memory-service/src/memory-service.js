...
const stm_engine_1 = require("./engines/stm-engine");
const episodic_memory_engine_1 = require("./engines/episodic-memory-engine");
const semantic_memory_engine_1 = require("./engines/semantic-memory-engine");
const reflection_engine_1 = require("../../reflection-service/src/reflection-engine");
let reflectionEngine = null;
function getReflectionEngine() {
    if (!reflectionEngine) {
        reflectionEngine = new reflection_engine_1.ReflectionEngine();
    }
    return reflectionEngine;
}
const database_1 = require("@agent-memory/database");
const logger = (0, shared_1.createLogger)('MemoryService');
class MemoryService {
    /**
    ...
    async storeMemory(input) {
        try {
            // Validate input
            const validatedInput = (0, shared_1.validate)(shared_1.memoryInputSchema, input);
            ...
            switch (validatedInput.type) {
                ...
                case 'episodic':
                    ...
                    if (validatedInput.importance) {
                        ...
                        await reflectionEngine.incrementAccumulated(validatedInput.agentId, validatedInput.importance);
                    ...
                    break;
                case 'semantic':
                    const semanticEngine = (0, semantic_memory_engine_1.getSemanticMemoryEngine)();
                    memoryId = await semanticEngine.storeSemanticMemory(validatedInput.agentId, validatedInput.content, validatedInput.metadata?.source || 'user', validatedInput.metadata?.category);
                    break;
                default:
    ...
    async retrieveMemories(query) {
        try {
            // Validate query
            const validatedQuery = (0, shared_1.validate)(shared_1.memoryQuerySchema, query);
            ...
            if (!validatedQuery.type || validatedQuery.type === 'episodic') {
                ...
                results.push(...episodicResults.map((r) => ({
                    id: r.id,
                    content: r.content,
            ...
            if (validatedQuery.type === 'short-term' && validatedQuery.sessionId) {
                ...
                results.push(...stmContents.map((content, index) => ({
     ...
     * Update memory
     */
    async updateMemory(id, _updates) {
        try {
    ...
    async createSession(agentId, metadata) {
        try {
            ...
            logger.error('Failed to create session', error, { agentId });
            throw error;
        }
    }
    /**
