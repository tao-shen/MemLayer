...
import { TacitDetailPanel } from './TacitDetailPanel';
...
export const TacitVisualizationDashboard: React.FC<DashboardProps> = ({
  agentId,
  initialView = 'timeline',
}) => {
  const { currentView, setCurrentView, filters, selectedTacit, setSelectedTacit, updateFilters, resetFilters } = useVisualizationStore();
  ...
  useKeyboardShortcuts([
    ...
    {
      key: 'Escape',
      callback: () => setSelectedTacit(null),
  ...
  const renderView = () => {
    ...
    switch (currentView) {
      case 'timeline':
        if (!timelineData || timelineData.tacits.length === 0) {
          return (
            <EmptyState
              title="No tacit knowledge in timeline"
              message="There are no tacit knowledge to display in the timeline view. Try adjusting your filters or create some tacit knowledge first."
              action={{
                label: 'Reset Filters',
        ...
        return (
          <TimelineView
            data={timelineData}
            onTacitClick={(tacit) => setSelectedTacit(tacit)}
          />
        );
      ...
      case 'graph':
        if (!graphData || graphData.nodes.length === 0) {
          return (
            <EmptyState
              title="No tacit knowledge in graph"
              message="There are no tacit knowledge to display in the graph view. Try adjusting your filters or create some tacit knowledge first."
              action={{
                label: 'Reset Filters',
                onClick: resetFilters,
              }}
        ...
        return (
          <GraphView
            data={graphData}
            onNodeClick={(node) => setSelectedTacit(node.tacit)}
          />
        );

      case 'list':
        if (!visualizationData || visualizationData.tacits.length === 0) {
          return (
            <EmptyState
              title="No tacit knowledge found"
              message="There are no tacit knowledge to display. Try adjusting your filters or create some tacit knowledge first."
              action={{
                label: 'Reset Filters',
                onClick: resetFilters,
              }}
        ...
        return (
          <ListView
            tacits={visualizationData.tacits}
            onTacitClick={(tacit) => setSelectedTacit(tacit)}
          />
        );

      case 'statistics':
        if (!statistics || statistics.summary.totalTacits === 0) {
          return (
            <EmptyState
              title="No statistics available"
              message="There are no tacit knowledge to generate statistics from."
            />
          );
        }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 px-4 md:px-6 py-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">
            {isMobile ? 'Tacit Knowledge' : 'Tacit Knowledge Visualization'}
          </h1>
      ...
      {selectedTacit && (
        <TacitDetailPanel
          tacit={selectedTacit}
          onClose={() => setSelectedTacit(null)}
          onNavigateToTacit={(tacitId) => {
            // TODO: Implement navigation to specific tacit knowledge
            console.log('Navigate to tacit knowledge:', tacitId);
          }}
        />
      )}
    </div>
