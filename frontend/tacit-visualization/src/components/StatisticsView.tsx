...
import { Statistics, TacitType } from '../types';
...
export const StatisticsView: React.FC<StatisticsViewProps> = ({ statistics }) => {
  ...
  const typeDistributionData = Object.entries(statistics.typeDistribution).map(
    ([type, count]) => ({
      ...
      value: count,
      color: TACIT_TYPE_COLORS[type as TacitType],
    })
  );
    ...
    <div className="space-y-6 p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm font-medium text-gray-500">Total Tacit Knowledge</div>
          <div className="mt-2 text-3xl font-bold text-gray-900">
            {statistics.summary.totalTacits}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm font-medium text-gray-500">Oldest Tacit Knowledge</div>
          <div className="mt-2 text-lg font-semibold text-gray-900">
            {statistics.summary.oldestTacit
              ? new Date(statistics.summary.oldestTacit).toLocaleDateString()
              : 'N/A'}
          </div>
        </div>

        <div className="bg-white rounded-lg shadow p-6">
          <div className="text-sm font-medium text-gray-500">Newest Tacit Knowledge</div>
          <div className="mt-2 text-lg font-semibold text-gray-900">
            {statistics.summary.newestTacit
              ? new Date(statistics.summary.newestTacit).toLocaleDateString()
              : 'N/A'}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Tacit Knowledge Type Distribution
          </h3>
        ...
        <div className="bg-white rounded-lg shadow p-6 lg:col-span-2">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            Tacit Knowledge Creation Over Time
          </h3>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={timeDistributionData}>
              ...
              <Line
                type="monotone"
                dataKey="stm"
                stroke={TACIT_TYPE_COLORS.stm}
                name="STM"
              />
              <Line
                type="monotone"
                dataKey="episodic"
                stroke={TACIT_TYPE_COLORS.episodic}
                name="Episodic"
              />
              <Line
                type="monotone"
                dataKey="semantic"
                stroke={TACIT_TYPE_COLORS.semantic}
                name="Semantic"
              />
              <Line
                type="monotone"
                dataKey="reflection"
                stroke={TACIT_TYPE_COLORS.reflection}
                name="Reflection"
              />
            </LineChart>
          </ResponsiveContainer>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {Object.entries(statistics.summary.byType).map(([type, count]) => (
                <tr key={type}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div
                        ...
                        style={{ backgroundColor: TACIT_TYPE_COLORS[type as TacitType] }}
                      />
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {((count / statistics.summary.totalTacits) * 100).toFixed(1)}%
                  </td>
                </tr>
              ))}
            </tbody>
