...
import { MemoryGraph, GraphNode, GraphEdge, EdgeType, MemoryType } from '../types';
...
export const GraphView: React.FC<GraphViewProps> = ({
  ...
  useEffect(() => {
    ...
    const node = g
      ...
      .on('mouseenter', (event, d) => {
        ...
        setTooltip({
          ...
          x: x + 10,
          y: y - 10,
          content: `${d.memory.type.toUpperCase()}\n${d.memory.content.substring(0, 100)}${d.memory.content.length > 100 ? '...' : ''}\nImportance: ${d.memory.importance || 'N/A'}`,
        });
      })
      .on('mouseleave', () => {
    ...
    node
      .append('circle')
      .attr('r', (d) => d.size)
      .attr('fill', (d) => MEMORY_TYPE_COLORS[d.memory.type])
      .attr('stroke', '#fff')
      .attr('stroke-width', 2);
