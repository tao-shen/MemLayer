...
import { TimelineData, TimelineMemory, MemoryType } from '../types';
...
export const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  useEffect(() => {
    if (!svgRef.current || !data.memories.length) return;
    ...
    nodes
      ...
      .attr('fill', (d) => MEMORY_TYPE_COLORS[d.type])
      .attr('stroke', '#fff')
      .attr('stroke-width', 2)
    ...
    memoryTypes.forEach((type, i) => {
      ...
      legendRow
        .append('circle')
        .attr('r', 6)
        .attr('fill', MEMORY_TYPE_COLORS[type]);
      ...
      legendRow
        ...
        .attr('y', 5)
        .attr('font-size', '12px')
        .text(`${MEMORY_TYPE_ICONS[type]} ${type.toUpperCase()}`);
    });
  ...
  }, [data, width, height, onMemoryClick]);
